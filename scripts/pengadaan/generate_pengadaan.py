...
def process_tahun(tahun):
    ...
    def bersihkan_dan_ambil_kanan(raw_rup_str):
        """
        Logika Kaku Sensor Bagi 2:
        1. Jika Ganjil -> Tolak, biarkan utuh.
        2. Jika Genap -> Ngintip (belah Kiri dan Kanan)
        3. Cek Relasi -> Jika Kiri dan Kanan punya riwayat, baru potong. Jika tidak, tolak.
        """
        if pd.isna(raw_rup_str) or not str(raw_rup_str).strip(): return ""
        parts = [p.strip() for p in str(raw_rup_str).split(';') if p.strip()]
        
        # Tolak jika ganjil atau kosong
        if len(parts) <= 1 or len(parts) % 2 != 0:
            return ";".join(parts)
        
        # Ngintip posisi Kiri dan Kanan
        mid = len(parts) // 2
        kiri = parts[:mid]
        kanan = parts[mid:]
        
        # Cek ikatan sejarah antara sisi Kiri dan Kanan
        if is_related_list(kiri, kanan):
            return ";".join(kanan) # Potong!
            
        return ";".join(parts) # Tolak, biarkan utuh
